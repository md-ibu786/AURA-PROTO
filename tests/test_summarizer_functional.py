"""
============================================================================
FILE: test_summarizer_functional.py
LOCATION: tests/test_summarizer_functional.py
============================================================================
...
ROLE IN PROJECT:
    Tests the functional behavior of the summarizer service.
    Ensures proper fallback between GenAI and Vertex AI models.
    - Key responsibility 1: Test GenAI path priority and fallback
    - Key responsibility 2: Validate configuration constants

KEY COMPONENTS:
    - TestSummarizerFunctional: Main test class
    - test_genai_path_is_attempted_first: Priority testing
    - test_genai_path_works_when_available: Availability testing

DEPENDENCIES:
    - External: unittest, pathlib, sys, os
    - Internal: services.summarizer

USAGE:
    Run with: pytest tests/test_summarizer_functional.py
    or: python -m pytest tests/test_summarizer_functional.py -v
============================================================================
"""
...
class TestSummarizerFunctional(unittest.TestCase):
    ...
    def test_genai_path_works_when_available(self, mock_get_genai_model):
        ...
        with patch(
            "services.genai_client.generate_content_with_thinking"
        ) as mock_genai_generate:
            mock_response = Mock()
    ...
    def test_function_handles_exceptions_gracefully(self):
        ...
        with patch(
            "services.summarizer.get_genai_model", side_effect=Exception("Test error")
        ):
            result = generate_university_notes(
                self.sample_topic, self.sample_transcript
            )
