"""
============================================================================
FILE: test_api_e2e_duplicates.py
LOCATION: tests/test_api_e2e_duplicates.py
============================================================================

PURPOSE:
    E2E duplicate module handling checks against running API server.

ROLE IN PROJECT:
    Validates that the API correctly handles duplicate module creation
    with automatic numbering and code assignment.
    - Tests module creation with duplicate names
    - Verifies sequential numbering and code generation
    - Ensures proper cleanup of test data

KEY COMPONENTS:
    - wait_for_server: Waits for API server to be available
    - test_duplicate_module_handling: Main E2E test for duplicate handling

DEPENDENCIES:
    - External: pytest, requests
    - Internal: None (tests against running API)

USAGE:
    Run with: pytest tests/test_api_e2e_duplicates.py -v
...
def test_duplicate_module_handling():
    ...
    # Create Dept
    res = requests.post(
        f"{BASE_URL}/departments", json={"name": "Test Dept E2E", "code": "TESTDEPT"}
    )
    if res.status_code == 200:
        ...
        res = requests.post(
            ...
            json={"name": f"Test Dept {rand_id}", "code": f"TD{rand_id}"},
        )
    ...
    res = requests.post(
        ...
        json={"department_id": dept_id, "semester_number": 1, "name": "Sem 1"},
    )
    try:
        res.raise_for_status()
    ...
    res = requests.post(
        f"{BASE_URL}/subjects",
        json={"semester_id": sem_id, "name": "Test Subject", "code": "TS101"},
    )
    res.raise_for_status()
    ...
    m1 = requests.post(
        f"{BASE_URL}/modules",
        json={"subject_id": subj_id, "module_number": 1, "name": "Module 1"},
    )
    m1.raise_for_status()
    m2 = requests.post(
        f"{BASE_URL}/modules",
        json={"subject_id": subj_id, "module_number": 2, "name": "Module 2"},
    )
    m2.raise_for_status()
    m3 = requests.post(
        f"{BASE_URL}/modules",
        json={"subject_id": subj_id, "module_number": 3, "name": "Module 3"},
    )
    m3.raise_for_status()
    ...
    m_new = requests.post(
        f"{BASE_URL}/modules",
        json={"subject_id": subj_id, "module_number": 99, "name": "Module Gap Fill"},
    )
    ...
    if new_num == 2:
        ...
        print(
            "SUCCESS: Sequential numbering worked (if that was the choice)! but we expected gap filling."
        )
    else:
