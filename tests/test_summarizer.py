...
PURPOSE:
    Unit tests for the summarizer functionality.
    Tests the generate_university_notes function with mocked dependencies.

ROLE IN PROJECT:
    Core test suite for the summarizer service.
    Validates basic functionality and error handling.
    - Key responsibility 1: Test GenAI availability check
    - Key responsibility 2: Test fallback to Vertex AI

KEY COMPONENTS:
    - TestSummarizer: Main test class
    - test_generate_university_notes_with_genai_available: GenAI path
    - test_generate_university_notes_fallback_vertexai: Fallback path

DEPENDENCIES:
    - External: unittest, importlib
    - Internal: services.summarizer

USAGE:
    Run with: pytest tests/test_summarizer.py
    or: python -m pytest tests/test_summarizer.py -v
============================================================================
"""
...
class TestSummarizer(unittest.TestCase):
    ...
    def test_generate_university_notes_with_genai_available(
        self, mock_get_model, mock_get_genai_model
    ):
    ...
    def test_generate_university_notes_fallback_vertexai(
        self, mock_generation_config, mock_get_model, mock_get_genai_model
    ):
        ...
        with patch("services.summarizer.generate_content") as mock_generate_content:
            ...
            result = generate_university_notes(
                self.sample_topic, self.sample_transcript
            )
            ...
            mock_generation_config.assert_called_with(
    ...
    @patch("services.summarizer.get_genai_model")
    def test_generate_university_notes_genai_exception_handling(
        self, mock_get_genai_model
    ):
    ...
    def test_generate_university_notes_vertexai_exception_handling(
        self, mock_generate_content, mock_get_model, mock_get_genai_model
    ):
