"""
============================================================================
FILE: test_audio_validation.py
LOCATION: tests/test_audio_validation.py
============================================================================
...
ROLE IN PROJECT:
    ...
    Ensures proper handling of unsupported formats, small files, and
    external service timeouts.
    - Validates file format restrictions
    - Tests error responses for invalid inputs
    - Verifies graceful handling of service failures

KEY COMPONENTS:
    - test_transcribe_with_unsupported_format: Tests format validation
    - test_transcribe_with_very_small_file: Tests minimum size validation
    - test_transcribe_deepgram_timeout: Tests timeout error handling

DEPENDENCIES:
    - External: pytest, fastapi.testclient, unittest.mock
    - Internal: api.main (FastAPI app)
...
def test_transcribe_deepgram_timeout(mock_process):
    ...
    mock_process.side_effect = Exception(
        "Deepgram transcription failed: Operation timed out"
    )
