 ...
 * ============================================================================
 * FILE: api.spec.ts
 * LOCATION: frontend/e2e/api.spec.ts
 * ============================================================================
 *
 * PURPOSE:
 ...
 *    - Internal: ApiHelper (page object), fixtures (test/expect exports)
 *
 * USAGE:
 *    Run all API tests: npx playwright test e2e/api.spec.ts
...
test.describe('API - Error Handling', () => {
    ...
    test('Returns 404 for non-existent department', async ({ request }) => {
        // Use request context directly to check error responses
        const response = await request.get('/api/departments/nonexistent-id-12345');
    ...
    test('Validates required fields', async ({ request }) => {
        // Use request context to test validation
