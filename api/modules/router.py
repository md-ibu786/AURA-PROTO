...
============================================================================
FILE: router.py
LOCATION: api/modules/router.py
============================================================================

PURPOSE:
    FastAPI router for M2KG Module CRUD and publishing endpoints, providing
    REST API endpoints for module management by staff users.

ROLE IN PROJECT:
    Part of the modules package handling module lifecycle management.
    Provides full CRUD operations and publishing workflow for course modules.
    - Key responsibility 1: Module CRUD operations (create, read, update, delete)
    - Key responsibility 2: Publishing workflow (publish/unpublish with audit)
...
KEY COMPONENTS:
    ...
    - create_module(), list_modules(), get_module(): CRUD endpoints
    - publish_module(), unpublish_module(): Publishing workflow

DEPENDENCIES:
    - External: fastapi
    - Internal: .service, .publishing, .models, config

USAGE:
    Mounted in main.py with /api/v1 prefix. Access via /api/v1/modules/*
============================================================================
"""
...
def list_modules(
    ...
    """
    result = service.list(
        status=status_filter, year=year, page=page, page_size=page_size
    )
    return result


@router.get("/{module_id}", response_model=ModuleResponse)
def get_module(module_id: str, service: ModuleService = Depends(get_module_service)):
    """
...
def delete_module(module_id: str, service: ModuleService = Depends(get_module_service)):
    """
