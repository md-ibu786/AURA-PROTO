...
def notes_by_module(department_id: int, semester_id: int, subject_id: int, module_id: int, limit: int = 20, offset: int = 0):
    ...
    count_q = "SELECT COUNT(*) as total FROM notes WHERE module_id = ?"
    total_row = execute_one(count_q, (module_id,))
    total = total_row['total'] if total_row else 0

    query = "SELECT id, title as label, 'note' as type, pdf_url, created_at FROM notes WHERE module_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?"
